...
function calculateRouteFromEtoF (platform) {
  var router = platform.getRoutingService(),
    routeRequestParams = {
      mode: 'fastest;truck',
      representation: 'display',
      waypoint0: '17.4603,78.5578', // moula ali
...
function enableTrafficInfo (map) {
  // Center map on New York
    map.setCenter({lat: 17.4802, lng: 78.5522});
    ...
    for (j = 0;  j < route.leg[i].maneuver.length; j += 1) {
      // Get the next maneuver.
            // console.log(route.leg[i].maneuver[j].position.latitude,route.leg[i].maneuver[j].position.longitude);
              ...
              array.push(route.leg[i].maneuver[j].position.latitude,route.leg[i].maneuver[j].position.longitude);

              // var latlng = new H.geo.Point(route.leg[i].maneuver[j].position.latitude,route.leg[i].maneuver[j].position.longitude);

              // marker_dutt1.setPosition(latlng);
              // boon();
              // setTimeout(marker_dutt1.setPosition(latlng),1000);
...
function transition(cords){
  i = 0;
  deltalat = (cords[0] - position[0])/numDeltas;
  deltalng = (cords[1] - position[1])/numDeltas;
  // moveMarker();
  boon();
}
...
function moveMarker(){
  ...
  // var latlng = new H.geo.Point(a[0],a[1]);
...
function boon(){
    ...
    // a = array[array_index].lat;
    // b = array[array_index].lng;

    // array_index++;
    console.log(array[array_index]);
    var latlng = new H.geo.Point(array[i],array[j]);
    marker_dutt1.setPosition(latlng);
